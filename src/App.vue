<template>
   <div id="app">
    ...
    <div class="container">
      ...
      <input type="search" @input="filtro = $event.target.value" placeholder="Buscar técnico pelo nome">
      ...
      <table>
        ...
        <tbody>
          ...
          <tr v-for="tecnico of filtrarTecnico" :key="tecnico.id" :id="'tec-'+tecnico.id">
            ...
            <td>
              <button @click="editar(tecnico)" class="waves-effect btn-small blue darken-1"><i class="material-icons">edit</i></button>
              <button @click.prevent="remover(tecnico)" class="waves-effect btn-small red darken-1"><i class="material-icons">delete</i></button>
            </td>
    ...
    methods : {
      ...
      salvar() {
        ...
        if(this.validaForm()) {
          ...
          // serão salvas.
          if(this.tecnico.id){
            Tecnico.atualizar(this.tecnico).then(resposta => {
            ...
            Tecnico.salvar(this.tecnico).then(resposta => {
              // Limpa os dados do formulário.
              this.tecnico = {};
              // Exibe uma mensagem.
      ...
      remover(tecnico) {
        ...
        if(confirm('Deseja realmente excluir este técnico?')){
          ...
          Tecnico.deletar(tecnico).then(tecnico => {
      ...
      validaForm() {
        ...
        if (this.tecnico.telefone.length < 13) {
          ...
          this.erro.telefone = 'Por favor, digite um telefone válido! Ex.: (99)99999-9999';
          // Acrescenta 1 ao conteúdo da variável 'erros'.
          erros++;
        }
